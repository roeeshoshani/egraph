...
impl ENode {
    ...
    pub fn to_effective(&self, union_find: &ENodesUnionFind) -> EffectiveENode {
...
impl ENodesUnionFind {
    ...
    pub fn does_enode_use_eclass(&self, enode: &ENode, used_eclass: EffectiveEClassId) -> bool {
    ...
    pub fn union_enodes(&mut self, a: ENodeId, b: ENodeId) -> UnionRes {
...
impl EGraph {
    ...
    pub fn add_graph(&mut self, graph: &Graph) -> GraphToEgraphTranslationMap {
        ...
        for graph_node_id in graph.valid_node_ids() {
            ...
            let add_res = self.add_graph_node(node, graph);
    ...
    /// adds a graph node to the egraph, recursively.
    fn add_graph_node(&mut self, node: &GraphNode, graph: &Graph) -> AddENodeRes {
        let enode = node.convert_links(|link_node_id| {
            self.add_graph_node(&graph[*link_node_id], graph).eclass_id
        });
